...
def _inject_messages(messages: list[dict], user_id: str, thread_id: str) -> list[dict]:
    ...
    # Rebrand Aura's own authoritative system prompt before inference, rather than only
    # rewriting visible response text after the model has already received a retired identity.
...
def install_aura_context_extensions() -> None:
    """Inject private context and the current brand into normal, streaming and regenerated Aura turns.

    The active user/thread scope is a ContextVar, so concurrent requests cannot inherit one
    another's profile/workspace context. Private tool-routing and summarisation prompts remain
    unaffected by member profile instructions.
    """
    global _INSTALLED
    if _INSTALLED:
        return
