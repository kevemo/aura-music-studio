...
from .daw import load_session, move_clip, public_session, save_session, set_automation, set_clip_fades, set_clip_gain, set_loop, set_track_mix
...
DAW_SPECS = [
    _spec("inspect_daw", "Read the pinned project's browser-safe DAW session: tracks, clips, mix state, automation, sends, markers and loop region.", {"project_name": "Project name/slug; omit when a project is pinned."}),
    _spec("set_daw_track_mix", "Non-destructively change one DAW track volume, pan, mute or solo. Track can be selected by id, exact/partial name or role. Volume may be absolute or relative.", {"project_name": "Project name/slug.", "track": "Track id, track name or role such as vocals/guitar/drums.", "volume_db": "Optional absolute volume in dB (-60 to +18).", "volume_delta_db": "Optional relative dB change, e.g. -3 means turn down 3 dB.", "pan": "Optional pan -1 left to +1 right.", "mute": "Optional boolean.", "solo": "Optional boolean."}, write=True),
    _spec("set_daw_clip_gain", "Non-destructively set or adjust gain for one DAW audio clip.", {"project_name": "Project name/slug.", "clip": "Clip id or exact/partial clip name.", "gain_db": "Optional absolute clip gain (-60 to +24 dB).", "gain_delta_db": "Optional relative gain change."}, write=True),
    _spec("set_daw_clip_fades", "Set non-destructive fade-in/fade-out lengths on one DAW clip.", {"project_name": "Project name/slug.", "clip": "Clip id or exact/partial name.", "fade_in": "Fade-in seconds.", "fade_out": "Fade-out seconds."}, write=True),
    _spec("move_daw_clip", "Move one DAW clip to a new timeline start without changing its source audio.", {"project_name": "Project name/slug.", "clip": "Clip id or name.", "start": "New timeline start in seconds."}, write=True),
    _spec("set_daw_loop", "Set or clear the DAW loop region.", {"project_name": "Project name/slug.", "start": "Loop start seconds or null to clear.", "end": "Loop end seconds or null to clear."}, write=True),
    _spec("set_daw_automation", "Replace one Pro DAW automation lane with explicit points.", {"project_name": "Project name/slug.", "track": "Track id/name/role.", "parameter": "Automation parameter such as volume_db or pan.", "points": "List of {time,value} points."}, write=True),
    _spec("render_daw_mix", "Render the current DAW session to a uniquely versioned real-audio WAV output without changing the editable session.", {"project_name": "Project name/slug."}, write=True),
]
...
def _explicit(tool_name: str, text: str) -> bool:
    ...
    if tool_name == "set_daw_track_mix":
        return any(x in lower for x in ("turn", "set", "change", "adjust", "lower", "raise", "increase", "decrease", "mute", "unmute", "solo", "pan"))
...
def _track_payload(item) -> dict[str, Any]:
    return {"id": item.id, "name": item.name, "role": item.role, "volume_db": item.volume_db, "pan": item.pan, "mute": item.mute, "solo": item.solo}
...
def install_aura_daw_tools() -> None:
    ...
    def execute(self, call: tools.ToolCall, *, latest_user_message: str):
        ...
        if call.name == "set_daw_track_mix":
            ...
            updated = set_track_mix(session, target.id, volume_db=volume, pan=args.get("pan"), mute=args.get("mute"), solo=args.get("solo"))
        ...
        if call.name == "set_daw_automation":
            ...
            lane = set_automation(session, target.id, parameter, args.get("points") or [])
            save_session(project, session)
        ...
        if call.name == "render_daw_mix":
            ...
            output = project / "output" / "daw" / f"{safe_name}_Aura_Chat_Mix_{render_id}.wav"
            render_session(session, project, output, project / "work" / f"daw_mix_{render_id}")
            relative = output.relative_to(project / "output").as_posix()
            return {"project_name": name, "rendered": True, "render_id": render_id, "output": relative, "real_audio_only": True, "storage_path_exposed": False, "versioned_output": True}
