...
def _install_specs() -> None:
    specs = [
        ...
        tools.ToolSpec(
            "bind_game_media_asset",
            "Assign one already-imported verified Game Forge media asset to an exact World DNA target. Use image assets for background/entity visuals/PBR textures, music/audio for soundtrack or entity_audio spatial sources, and video for cutscene.",
            {
                "game_id": "Stable Game Forge id.",
        ...
        tools.ToolSpec(
            "apply_game_media_command",
            "Apply a plain-language Game Forge media instruction such as 'use Cosmic Sky as the world background', 'set Sparkles as the soundtrack', 'apply Neon Stone to ground base color', 'use Waterfall Roar as spatial audio on Waterfall', or 'remove the cutscene'. Fails safely with candidates when the asset/entity is ambiguous.",
            {"game_id": "Stable Game Forge id.", "command": "Explicit member instruction for imported game media/bindings."}, write=True,
        ),
    ]
...
def _explicit_game_write_allowed(name: str, message: str) -> bool:
    ...
    media_terms = ("game", "background", "soundtrack", "cutscene", "texture", "visual", "player", "asset", "media", "audio", "sound effect", "sfx")
    if name == "bind_game_media_asset":
        return _has_intent_phrase(message, ("use", "set", "apply", "bind", "assign", "change")) and _has_intent_phrase(message, media_terms)
    if name == "unbind_game_media_asset":
        return _has_intent_phrase(message, ("remove", "clear", "unbind", "detach", "stop using")) and _has_intent_phrase(message, media_terms)
    if name == "apply_game_media_command":
        return _has_intent_phrase(message, ("use", "set", "apply", "bind", "assign", "change", "remove", "clear", "unbind", "show", "list")) and _has_intent_phrase(message, media_terms)
    return True
...
def install_aura_game_tools() -> None:
    ...
    def execute(self, call: tools.ToolCall, *, latest_user_message: str) -> Any:
        ...
        write_names = {"create_game_project", "scan_game_project", "build_game_playtest", "bind_game_media_asset", "unbind_game_media_asset", "apply_game_media_command"}
        ...
        if call.name == "inspect_game_project":
            ...
            return {"game_dna": game.model_dump(mode="json"), "assets": _asset_summary(game.id), "asset_bindings": binding_state(game.id)}
        ...
        game = load_game(str(args.get("game_id") or ""))
        if call.name in {"scan_game_project", "build_game_playtest", "bind_game_media_asset", "unbind_game_media_asset", "apply_game_media_command"}:
