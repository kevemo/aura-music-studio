...
def live_guardian(request: Request):
    ...
    audit_rows = "".join(
        ...
        f"<td>{escape(event.event_type.replace('_', ' ').title())}</td>"
        f"<td>{escape(event.actor)}</td>"
        f"<td><code>{escape(json.dumps(event.metadata, sort_keys=True, ensure_ascii=False))}</code></td>"
        "</tr>"
        for event in events
    ) or "<tr><td colspan='4' class='muted'>No Aura LIVE moderation events yet.</td></tr>"

    options = "".join(
        f"<option value='{item.value}'{' selected' if mode == item.value else ''}>{escape(item.value.replace('_', ' ').title())}</option>"
        for item in ModerationMode
    )
...
<div class='card'><h2>LIVE creator tools</h2><div class='row'><a class='btn primary' href='/live-overlay-studio'>Overlay Studio</a><a class='btn' href='/live-overlay-studio/prompter'>Auto Cue Prompter</a><a class='btn' href='/live-guardian/policy'>Moderation Policy</a></div><p class='muted'>The public overlay source never receives the private prompter script. Your moderation policy can tune lower-risk handling but cannot disable threat, doxxing or grooming protection.</p></div><div class='card'><h2>Audit integrity</h2><p>Moderation audit chain: <b class='{'safe' if chain_ok else 'warn'}'>{'Verified' if chain_ok else 'Integrity check failed'}</b></p><p class='muted'>Authorization and moderation evidence is append-only and hash chained. TikTok passwords, session cookies, access tokens, private keys and provider secrets are not stored here.</p></div></section></div>
...
def save_live_guardian_settings(
    request: Request,
    creator_handle: str = Form(...),
    mode: ModerationMode = Form(ModerationMode.ADVISORY),
    creator_consent: bool = Form(False),
    moderator_assignment_confirmed: bool = Form(False),
):
    member = _member(request)
    authorization = AuraModeratorAuthorization(
        creator_handle=creator_handle,
        creator_consent=creator_consent,
        moderator_assignment_confirmed=moderator_assignment_confirmed,
        mode=mode,
