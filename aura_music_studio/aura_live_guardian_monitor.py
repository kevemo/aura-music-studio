...
async function refresh(){try{const response=await fetch('/live-guardian/status',{credentials:'same-origin',cache:'no-store',headers:{'Accept':'application/json'}});if(!response.ok)throw new Error('status unavailable');const data=await response.json();state.textContent=data.safety_state.toUpperCase();state.className='pill '+data.safety_state;message.textContent=data.message;pending.textContent=String(data.pending_reviews);critical.textContent=String(data.critical_escalations);audit.textContent=data.audit_integrity_ok?'Verified':'Attention';provider.textContent=data.provider_execution_ready?'Runtime verified':'Fail-closed';}catch(error){state.textContent='OFFLINE';state.className='pill attention';message.textContent='Aura Guardian status is temporarily unavailable. No provider authority is inferred.';provider.textContent='Fail-closed';}}
