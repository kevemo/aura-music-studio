...
def _categories(values: set[str] | frozenset[str] | list[str] | tuple[str, ...] | None) -> frozenset[str]:
    ...
    # High-risk protections are platform safety boundaries and cannot be disabled by a creator.
...
class AuraLiveGuardianPolicyStore:
    ...
    def save(
        ...
        *,
        user_id: str,
        blocked_phrases: list[str] | tuple[str, ...] | None,
        language_tolerance: LanguageTolerance = "balanced",
        spam_sensitivity: SpamSensitivity = "medium",
        enabled_categories: set[str] | frozenset[str] | list[str] | tuple[str, ...] | None = None,
        actor: str,
        now: datetime | None = None,
    ) -> AuraLiveGuardianPolicy:
    ...
    def get(self, user_id: str) -> AuraLiveGuardianPolicy | None:
        ...
        with self._connect() as connection:
            row = connection.execute(
                "SELECT * FROM aura_live_guardian_policies WHERE user_id=?", (user_id,)
            ).fetchone()
        if row is None:
            return None
