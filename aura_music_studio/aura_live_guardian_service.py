...
def decide_and_record_guardian_event(
    *,
    store: AuraLiveModeratorStore,
    user_id: str,
    ...
    message: str = "",
    actor: str = "aura:live-guardian",
) -> AuditedGuardianDecision:
    """Make a bounded Guardian decision and append non-content audit evidence.

    Raw LIVE message text, blocked-phrase text and classifier evidence strings are deliberately not
    copied into the durable audit metadata. The audit captures decision/category/severity state and
    only the count of creator blocked-phrase matches.
    """
    ...
    event_type = (
        "human_escalation"
        if result.decision.action is ModerationAction.ESCALATE
