...
def emergency_snapshot(user_id: str) -> dict:
    ...
    if row:
        ...
        mode, reason, revision, updated_at = "normal", "", 0, None
    return {
        "mode": mode,
        "reason": reason,
        "revision": revision,
...
def create_show_plan(body: ShowPlanCreate, request: Request):
    ...
    with _connect() as con:
        con.execute(
            ...
            (plan_id, member.user_id, body.title.strip(), body.description.strip(), now, now),
        )
        row = _plan_row(con, member.user_id, plan_id)
        return _plan_payload(con, row)
    ...
    with _connect() as con:
        ...
        con.execute(
            ...
            (
                ...
                body.scene_name.strip() if body.scene_name else None,
                body.cue_label.strip() if body.cue_label else None,
                now,
                now,
            ),
        )
...
def update_show_segment(plan_id: str, segment_id: str, body: ShowSegmentUpdate, request: Request):
    ...
    values = body.model_dump(exclude_none=True)
    expected = int(values.pop("expected_revision"))
    for key in ("title", "scene_name", "cue_label"):
        if key in values and isinstance(values[key], str):
            values[key] = values[key].strip() or (None if key != "title" else values[key].strip())
    with _connect() as con:
        con.execute("BEGIN IMMEDIATE")
        plan = _plan_row(con, member.user_id, plan_id)
        ...
        row = con.execute(
            "SELECT id FROM live_show_segments WHERE id=? AND plan_id=? AND user_id=?",
        ...
        if "ordinal" in values:
            ...
            current = int(con.execute("SELECT ordinal FROM live_show_segments WHERE id=?", (segment_id,)).fetchone()[0])
            if wanted != current:
                con.execute("UPDATE live_show_segments SET ordinal=-1 WHERE id=?", (segment_id,))
                if wanted < current:
                    con.execute(
                        "UPDATE live_show_segments SET ordinal=ordinal+1 WHERE plan_id=? AND ordinal>=? AND ordinal<?",
                        (plan_id, wanted, current),
                    )
                else:
                    con.execute(
                        "UPDATE live_show_segments SET ordinal=ordinal-1 WHERE plan_id=? AND ordinal>? AND ordinal<=?",
                        (plan_id, current, wanted),
                    )
                con.execute("UPDATE live_show_segments SET ordinal=? WHERE id=?", (wanted, segment_id))
        if values:
            values["updated_at"] = _now()
            assignments = ",".join(f"{key}=?" for key in values)
            con.execute(
...
def delete_show_segment(plan_id: str, segment_id: str, request: Request, expected_revision: int):
    ...
    with _connect() as con:
        ...
        con.execute("UPDATE live_show_segments SET ordinal=ordinal-1 WHERE plan_id=? AND ordinal>?", (plan_id, ordinal))
        con.execute(
            "UPDATE live_show_plans SET revision=revision+1,updated_at=? WHERE id=? AND user_id=?",
...
def change_emergency_state(body: EmergencyCommand, request: Request):
    ...
    with _connect() as con:
        ...
        if prior_command:
            if str(prior_command["requested_mode"]) != body.mode:
