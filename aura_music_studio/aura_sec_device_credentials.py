...
class AuraSecDeviceCredentialRotation:
        ...
        with self._connect() as con:
            ...
            con.execute(
                """INSERT INTO aura_sec_device_key_rotations
                   ...
                   VALUES (?,?,?,?,?,?,?,?,?,'pending',?,?)""",
                (
                    challenge_id,user_id,device_id,approved_action_id,_hash_secret(challenge),
                    device["public_key_fingerprint"],new_fingerprint,str(device["platform"]),
                    str(device["architecture"]),_iso(now),_iso(expires),
                ),
            )
        return {
    ...
    def _reserve(self, user_id: str, challenge_id: str, challenge: str, *, now: datetime):
        ...
        with self._connect() as con:
            ...
            if now >= expires:
                con.execute("UPDATE aura_sec_device_key_rotations SET status='expired' WHERE id=?", (challenge_id,))
    ...
    def _release_failed_attempt(self, challenge_id: str, attempt_id: str) -> None:
        ...
        with self._connect() as con:
            row = con.execute(
                "SELECT expires_at FROM aura_sec_device_key_rotations WHERE id=? AND status='verifying' AND attempt_id=?",
    ...
    def _verify(self, item: dict, challenge: str, *, old_signature_b64: str, new_signature_b64: str,
                verifier: CredentialRotationVerifier | None) -> VerifiedCredentialRotation:
        if verifier is None:
            raise PermissionError("A trusted Aura Sec credential rotation verifier is required")
        context = CredentialRotationContext(
            user_id=str(item["user_id"]),device_id=str(item["device_id"]),
            approved_action_id=str(item["approved_action_id"]),challenge_id=str(item["id"]),
            challenge=challenge,old_public_key_fingerprint=str(item["old_public_key_fingerprint"]),
            new_public_key_fingerprint=str(item["new_public_key_fingerprint"]),platform=str(item["platform"]),
            architecture=str(item["architecture"]),created_at=str(item["created_at"]),expires_at=str(item["expires_at"]),
        )
        ...
        try:
            proof = verifier(context.old_public_key_fingerprint, context.new_public_key_fingerprint,
                             payload, old_signature, new_signature)
        except Exception as exc:
        ...
        return VerifiedCredentialRotation(old_fp,new_fp,verifier_id,old_alg,new_alg,evidence_digest,bool(proof.new_key_hardware_backed))

    @staticmethod
    def _table_exists(con: sqlite3.Connection, table_name: str) -> bool:
        return con.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name=?", (table_name,)).fetchone() is not None

    def complete_rotation(self, user_id: str, challenge_id: str, *, challenge: str,
                          old_signature_b64: str, new_signature_b64: str,
                          verifier: CredentialRotationVerifier | None, now: datetime | None = None) -> dict:
        ...
        try:
            ...
            if device.get("status") == "revoked" or device.get("revoked_at"):
                raise PermissionError("Revoked Aura Sec device cannot rotate credentials")
            if not secrets.compare_digest(device["public_key_fingerprint"], str(item["old_public_key_fingerprint"])):
                raise PermissionError("Aura Sec device credential changed after this challenge was issued")
            self._approved_rotation_action(user_id, str(item["device_id"]), str(item["approved_action_id"]))
            proof = self._verify(item, challenge, old_signature_b64=old_signature_b64,
                                 new_signature_b64=new_signature_b64, verifier=verifier)
            with self._connect() as con:
                cursor = con.execute(
                    """UPDATE aura_sec_devices
                       SET public_key_fingerprint=?,protection_state='awaiting_heartbeat',last_seen_at=NULL,
                           last_policy_version=NULL,last_report_digest=NULL,last_heartbeat_sequence=0,
                           last_heartbeat_verifier_id=NULL,last_heartbeat_key_algorithm=NULL,
                           last_heartbeat_evidence_digest=NULL
                       WHERE user_id=? AND id=? AND public_key_fingerprint=? AND revoked_at IS NULL""",
                    (proof.new_public_key_fingerprint,user_id,item["device_id"],item["old_public_key_fingerprint"]),
                )
                if cursor.rowcount != 1:
                ...
                if self._table_exists(con, "aura_sec_native_poll_state"):
                    con.execute("DELETE FROM aura_sec_native_poll_state WHERE user_id=? AND device_id=?",
                ...
                completed = con.execute(
                    """UPDATE aura_sec_device_key_rotations
                       SET status='consumed',attempt_id=NULL,consumed_at=?,verifier_id=?,old_key_algorithm=?,
                           new_key_algorithm=?,evidence_digest=?,new_key_hardware_backed=?
                       WHERE id=? AND user_id=? AND status='verifying' AND attempt_id=?""",
                    (_iso(current),proof.verifier_id,proof.old_key_algorithm,proof.new_key_algorithm,
                     proof.evidence_digest,1 if proof.new_key_hardware_backed else 0,challenge_id,user_id,attempt_id),
                )
        ...
        return {
            ...
            "verification": {
                "verifier_id": proof.verifier_id,"old_key_algorithm": proof.old_key_algorithm,
                "new_key_algorithm": proof.new_key_algorithm,"evidence_digest": proof.evidence_digest,
...
__all__ = ["AuraSecDeviceCredentialRotation", "CredentialRotationContext", "CredentialRotationVerifier", "VerifiedCredentialRotation"]
