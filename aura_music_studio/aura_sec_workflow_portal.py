...
def approvals_page(request: Request):
    ...
    rows = "".join(
        f"<div class='row'><div><b>{escape(_label(item['action_type']))}</b><div class='meta'>{escape(str(item.get('display_name') or 'Device'))} · Risk {escape(_label(item['risk_class']))} · Proposed {escape(str(item['requested_at']))}</div></div><div><span class='pill {_tone(item['status'])}'>{escape(_label(item['status']))}</span><br><a class='action' href='/aura-sec/approval/{escape(str(item['id']))}'>Review approval</a></div></div>"
        for item in proposed
    ) or "<div class='empty'>No bounded security actions are currently awaiting approval.</div>"
    body = f"""<div class='grid'><div class='card metric'><strong>{len(proposed)}</strong><span>proposed actions</span></div><div class='card metric'><strong>{high_risk}</strong><span>requiring strong re-authentication</span></div><div class='card metric'><strong>0</strong><span>browser-executable generic commands</span></div></div><section class='section panel'><div class='eyebrow'>Human approval boundary</div><h2>Actions awaiting review</h2>{rows}</section><section class='section truth'><b>Approval and execution remain separate.</b> The secure approval gateway binds a one-time challenge to this login session and exact action; high-risk actions require password re-authentication. Only after approval can a later signed native-device session request the typed command.</section>"""
