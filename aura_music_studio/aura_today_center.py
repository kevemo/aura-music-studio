...
router = APIRouter(tags=["Rhian Today"])
store = AuraChatStore()
...
def today_snapshot(request: Request, thread_id: str = "", limit: int = 6):
    ...
    try:
        ...
        raise HTTPException(404, "Rhian conversation not found") from exc
...
(()=>{
  ...
  function ensureDrawer(){
    ...
    d.innerHTML=`<div style="display:flex;align-items:center;gap:8px"><div style="flex:1"><b style="font-size:1.05rem">Rhian Today</b><div class="muted" style="font-size:.7rem">Your private at-a-glance workspace</div></div><button class="btn" id="auraTodayClose">✕</button></div><div style="display:flex;gap:6px;margin:12px 0"><button class="mini" id="auraTodayRefresh">Refresh</button><button class="mini" id="auraTodayBrief">Ask Rhian for a full brief</button></div><div id="auraTodayEventDetail"></div><div id="auraTodayRows"></div>`;
  ...
  function render(data){
    ...
    const header=`<div style="padding:11px;border:1px solid #ffffff18;border-radius:14px;background:linear-gradient(135deg,#9b70ff16,#58dfff0c)"><div style="display:flex;justify-content:space-between;gap:8px"><b>${connected?'Connected workspace':'Rhian workspace'}</b><span class="muted" style="font-size:.68rem">${data.unread_notification_count||0} unread</span></div><div class="muted" style="font-size:.7rem;margin-top:5px">${pinned?'📌 '+esc(pinned):'No creative project pinned'} · read-only overview</div></div>`;
    const noGoogle=connected?'':card('Google services','<p class="muted">Google is not connected to Rhian. Tasks, notifications and your pinned project still work here. Open Connectors to add read-only Calendar/Gmail/Drive access.</p>');
    ...
    $('auraTodayRows').innerHTML=header+noGoogle+
      ...
      card('⏰ Active Rhian Tasks',taskRows(data.tasks||[]))+
  ...
  const foot=document.querySelector('.sideFoot');if(foot&&!$('auraTodayButton')){const b=document.createElement('button');b.id='auraTodayButton';b.className='btn';b.textContent='☀ Rhian Today';b.onclick=async()=>{ensureDrawer().classList.add('open');await loadToday()};foot.prepend(b)}
})();
"""
