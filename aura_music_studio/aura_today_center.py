...
(()=>{
  ...
  function ensureDrawer(){
    ...
    d.addEventListener('click',async event=>{
      ...
      const mail=event.target.closest('[data-aura-mail]');if(mail&&typeof send==='function'){d.classList.remove('open');send(`Read the Gmail message with id ${mail.dataset.auraMail} and summarize what I need to know. Do not send or modify anything.`)}
    });
    return d;
  }
