...
from .branding import BRAND_ARTWORK_FILENAME, BRAND_ICON_PATH, BRAND_LOGO_PATH, ENDORSEMENT, PRODUCT_FULL_NAME, TAGLINE
...
_BRAND_ARTWORK_FILE = Path(__file__).resolve().parent / "static" / BRAND_ARTWORK_FILENAME
_LEGACY_LOGO_PATHS = {
    "/brand/pulsar-frequency-house-logo.svg",
    "/static/pulsar-frequency-house-logo.svg",
}

# Keep the historical data-pfh marker alongside the canonical marker because older presentation
# integrations use it as non-public compatibility metadata.
...
_REPLACEMENTS: tuple[tuple[str, str], ...] = (
    ...
    # Retired visual asset routes. Existing templates can keep old paths in source while the
    # public response resolves to the supplied master artwork.
    ...
    ("The platform is being built around continuity:", "The platform is built around Creative DNA continuity:"),
    (
        ...
        "The real-audio music engine, Creative DNA layer, Aura Core, owner controls and Elevate Souls Productions permission systems are connected under the master brand. Image/video renderer bridges are present while their external generation backends remain deployment-configurable.",
    ),
)
...
def rebrand_text(value: str) -> str:
    ...
    if "data-esp-command-center-aura-core='0.20'" not in value and _LANDING_MEMBERSHIP_MARKER in value:
        value = value.replace(_LANDING_MEMBERSHIP_MARKER, _AURA_CORE_HOME_SECTION + _LANDING_MEMBERSHIP_MARKER, 1)
    return value
...
class BrandMigrationMiddleware(BaseHTTPMiddleware):
    """Serve the master artwork and rewrite retired public-facing brand copy.

    Binary audio/video/image responses are otherwise passed through untouched. The two old
    logo routes are intercepted before routing so mature templates cannot surface the retired
    visual even when their source URL has not yet been changed.
    """

    async def dispatch(self, request: Request, call_next):
        if request.method.upper() in {"GET", "HEAD"} and request.url.path in ({BRAND_LOGO_PATH} | _LEGACY_LOGO_PATHS):
            if _BRAND_ARTWORK_FILE.exists():
                return FileResponse(_BRAND_ARTWORK_FILE, media_type="image/webp", filename=BRAND_ARTWORK_FILENAME)
