...
class CreativeEffectEntitlementStore:
    ...
    def purchase(self, user_id: str, effect_id: str, *, idempotency_key: str, actor: str = "member") -> dict:
        ...
        with self._connect() as con:
            ...
            if existing_tx:
                # The wallet debit and entitlement are designed to be atomic. Seeing the debit
                # without an active entitlement means the database was externally altered.
            ...
            con.execute(
                ...
                (
                    ...
                    new_balance,
                    "spend",
                    f"Permanent {item.entitlement} creative effect unlock: {item.id}",
                    reference,
                    actor,
                    now,
                ),
            ...
            previous = con.execute(
                "SELECT id FROM creative_effect_entitlements WHERE user_id=? AND effect_id=?",
                (user_id, item.id),
            ).fetchone()
            if previous:
                con.execute(
        ...
        *,
        ...
        with self._connect() as con:
            ...
            existing = con.execute(
                "SELECT * FROM credit_transactions WHERE reference=?", (refund_reference,)
            ).fetchone()
            if existing:
            ...
            con.execute(
                """UPDATE creative_effect_entitlements
                   SET active=0,updated_at=?,revoked_at=?,revoked_reason=? WHERE id=?""",
                (now, now, reason, entitlement["id"]),
            )
            return {
                "refunded": True,
...
__all__ = [
    "CreativeEffectEntitlementStore",
    "LEGACY_LEDGER_UNIT",
    "PUBLIC_COIN_UNIT",
    "store",
]
