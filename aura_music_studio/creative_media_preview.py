...
(()=>{
  ...
  function renderRows(){
    ...
    target.innerHTML=items.length?items.map(e=>`<div class="item" style="margin:8px 0"><div class="itemtop"><div><b>${esc(e.label||e.kind)}</b><div><span class="chip">${esc(e.kind)}</span><span class="chip ${e.status==='ready'?'good':'wait'}">${esc(e.status||'draft')}</span>${versionChip(e)}</div><div class="source">${esc(e.source_ref)}</div></div><div style="display:flex;gap:5px;flex-wrap:wrap;justify-content:flex-end"><button class="btn small" data-creative-preview="${esc(e.id)}">Preview</button>${!isCurrent(e)&&e.status!=='archived'?`<button class="btn small" data-creative-activate="${esc(e.id)}">Make current</button>`:''}</div></div></div>`).join(''):'<div class="empty">No local image, video or audio outputs are registered in this project yet. Generate/import media first, then it will appear here.</div>';
