...
class CreditWalletStore:
    """Durable credit ledger isolated from subscriptions and ESP role permissions.

    Credits are a commercial entitlement only. They never grant Creator, Agent,
    Creator+Agent, Owner, or any other ESP-internal role.
    """
    ...
    def _ensure_wallet(self, con: sqlite3.Connection, user_id: str) -> None:
        now = _iso()
        con.execute(
    ...
    def adjust(
        ...
        *,
        ...
        kind = (kind or "adjustment").strip().lower()[:40]
        reason = (reason or "Credit adjustment").strip()[:240]
        actor = (actor or "system").strip()[:120]
...
class AdminCreditGrantRequest(BaseModel):
    ...
    actor: str = Field(default="ESP Owner", min_length=2, max_length=120)
