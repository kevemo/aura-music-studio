...
def level_up_with_agent_roster(request: Request):
    """Compose the assigned-roster and health-queue Agent OS surfaces."""
    ...
    if marker in html and "/command-center/agent/roster" not in html:
        html = html.replace(marker, marker + roster_link + health_link, 1)
    elif roster_link in html and "/command-center/agent/health" not in html:
