...
def _canonical_metrics(values: dict | None) -> dict[str, int | float]:
    source = {_normalise_key(k): v for k, v in (values or {}).items()}
...
def extract_structured_metrics(filename: str, content: bytes) -> tuple[dict, str]:
    """Extract metrics only from transparent text formats.

    Screenshots/PDFs intentionally do not use hidden/fake OCR. They are retained for a
    later configured Aura/vision extraction step or human confirmation.
    """
    ...
    try:
        if suffix == ".json":
            ...
            if not isinstance(payload, dict):
                payload = {}
            metrics = _canonical_metrics(payload.get("metrics") if isinstance(payload.get("metrics"), dict) else payload)
        elif suffix == ".csv":
            reader = csv.DictReader(io.StringIO(text))
            first = next((row for row in reader if any(str(v or "").strip() for v in row.values())), {})
            metrics = _canonical_metrics(first)
        else:
            parsed: dict[str, str] = {}
            ...
            metrics = _canonical_metrics(parsed)
...
def _trend(current: dict, previous: dict | None) -> dict:
    ...
    for key, value in current.items():
        now_value = _number(value)
        before = _number(prior.get(key))
        if now_value is None or before is None:
            continue
        result[key] = {"current": now_value, "previous": before, "delta": round(now_value - before, 2)}
    return result
...
class BackstageEvidenceStore:
    ...
    def record(
        ...
        *,
        ...
        self._authorize(actor_user_id, creator_user_id, owner=owner)
            ...
            con.execute(
                """INSERT INTO esp_backstage_evidence
                   (id,creator_user_id,uploaded_by_user_id,source_kind,source_label,captured_at,period_label,
                    ...
                    progress_submission_id,created_at)
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (
    ...
    def get(self, evidence_id: str, *, actor_user_id: str, owner: bool = False) -> dict:
        ...
        self._authorize(actor_user_id, item["creator_user_id"], owner=owner)
        return item
    ...
    def list_for_creator(self, actor_user_id: str, creator_user_id: str, *, owner: bool = False, limit: int = 100) -> list[dict]:
        ...
        for index, item in enumerate(result):
            ...
            item["trend"] = _trend(item.get("metrics") or {}, previous)
        return result
    ...
    def queue(self, actor_user_id: str, *, owner: bool = False) -> list[dict]:
        ...
        if owner:
            with self._connect() as con:
                rows = con.execute(
                    """SELECT user_id FROM esp_memberships
                       WHERE status IN ('active','owner') AND (roles IN ('creator','both') OR status='owner')"""
                ).fetchall()
        ...
        with self._connect() as con:
            for creator_id in creator_ids:
                ...
                item = {
                    ...
                    "latest": self._decode(latest) if latest else None,
                }
                item["freshness"] = _freshness(
                    (item["latest"] or {}).get("captured_at"),
                    (item["latest"] or {}).get("created_at"),
                ) if item["latest"] else {"status": "missing", "age_days": None}
                queue.append(item)
        order = {"missing": 0, "stale": 1, "update_due": 2, "unknown": 3, "current": 4}
        queue.sort(key=lambda row: (order.get(row["freshness"]["status"], 9), row["display_name"].lower()))
        return queue
...
async def save_backstage_evidence(
    ...
    comments: str = Form(""), shares: str = Form(""), likes: str = Form(""), diamonds: str = Form(""), gifters: str = Form(""),
...
__all__ = [
    "router", "BackstageEvidenceStore", "backstage_evidence", "extract_structured_metrics",
