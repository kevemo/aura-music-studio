...
class ExportProvenanceStore:
        ...
        *,
        ...
        with self._connect() as con:
            duplicate = con.execute(
                "SELECT 1 FROM creative_export_provenance WHERE sha256=? LIMIT 1", (digest,)
            ...
            con.execute(
                ...
                """,
                (
                    export_id, user_id, project_name, sequence_id, filename, media_kind, format, digest,
                    int(commercial_use_requested), int(rights_attested), int(duplicate), _iso(),
                ),
            )
    ...
    def get_for_user(self, export_id: str, user_id: str) -> dict:
        with self._connect() as con:
            row = con.execute(
                "SELECT * FROM creative_export_provenance WHERE id=? AND user_id=?", (export_id, user_id)
            ).fetchone()
        if not row:
    ...
    def _public(row: dict) -> dict:
        ...
        row["similarity_scope"] = (
            "external_review_recorded" if row["external_similarity_completed"] else "internal_exact_sha256_only"
        )
        ...
        row["commercial_platform_export_allowed"] = bool(
            row["commercial_use_requested"]
            and row["rights_attested"]
            and row["review_status"] == "cleared_for_platform_export"
        )
        return row
...
__all__ = [
    "member_router", "owner_router", "store", "ExportProvenanceStore", "OwnerExportReviewInput", "sha256_file"
]
