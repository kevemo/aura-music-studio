...
from .game_forge_export import _MAX_EXPORT_ASSETS, _MAX_EXPORT_MEDIA_BYTES
...
def _enforce_export_budget(game_id: str) -> dict:
    rows = runtime_asset_manifest(game_id)
    if len(rows) > _MAX_EXPORT_ASSETS:
    ...
    for row in rows:
        ...
        total += size
        if total > _MAX_EXPORT_MEDIA_BYTES:
            raise ValueError(
...
def _capability(game_id: str) -> dict:
    return {
        ...
        "reason": (
            ...
            "and production release signing is independently verified."
        ),
    }
...
def create_godot_source_preview(game_id: str, request: Request):
    ...
    game = _game(game_id)
    try:
        budget = _enforce_export_budget(game_id)
