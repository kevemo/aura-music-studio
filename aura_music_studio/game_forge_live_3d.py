...
function snapshot3(){return Array.from(hidden3.entries()).map(([id,value])=>({id,kind:value.kind,taken:value.taken===true,scale:{...value.scale}}))}
...
const undoButton3=document.createElement('button');undoButton3.id='aura-live-3d-undo';undoButton3.type='button';undoButton3.textContent='Undo 3D';undoButton3.title='Undo the last Aura3D authored-entity live tuning change';row3.appendChild(undoButton3);undoButton3.addEventListener('click',event=>{event.preventDefault();event.stopImmediatePropagation();undo3()});
...
window.AuraLive3D=Object.freeze({version:1,command:command3,undo:undo3,contract:Object.freeze({private_playtest_only:true,authored_entities_only:true,arbitrary_entity_creation:false,project_persistence:false,max_tunable_entities:max3})});
})();
</script>
"""
