...
from .game_forge_ratings import rating_content_hash
...
def render_foundation_playtest(game: GameDNA) -> str:
    """Generate a curated no-network browser prototype, never arbitrary creator/LLM JavaScript.

    Engine-specific Phaser/PlayCanvas/Babylon/Godot adapters can replace this runtime in later
    stages. Keeping the foundation deterministic lets us prove entitlement, sandbox, rating and
    publish boundaries before generated code is introduced.
    """
...
'use strict';const cfg={config};const canvas=document.getElementById('game'),ctx=canvas.getContext('2d',{{alpha:false}});document.getElementById('title').textContent=cfg.title;document.getElementById('meta').textContent=`${{cfg.genre}} · ${{cfg.dimension.toUpperCase()}} concept · target ${{cfg.engine_target}}`;
...
def build_private_playtest(game: GameDNA) -> tuple[GameDNA, str]:
    ...
    build = GameBuild(content_hash=content_hash, requested_engine=game.engine_target)
