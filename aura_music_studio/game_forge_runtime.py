...
const media=Array.isArray(cfg.assets)?cfg.assets:[];const role=a=>String(a?.role||'').toLowerCase();const firstImage=media.find(a=>a.kind==='image'&&/(background|backdrop|scene|world|environment)/.test(role(a)))||media.find(a=>a.kind==='image');const firstAudio=media.find(a=>(a.kind==='music'||a.kind==='audio')&&/(soundtrack|music|ambient|theme|score)/.test(role(a)))||media.find(a=>a.kind==='music'||a.kind==='audio');const firstVideo=media.find(a=>a.kind==='video'&&/(cutscene|intro|cinematic|background)/.test(role(a)))||media.find(a=>a.kind==='video');
let backgroundImg=null;if(firstImage){{const img=new Image();img.decoding='async';img.onload=()=>{{backgroundImg=img}};img.src=firstImage.media_url}}
