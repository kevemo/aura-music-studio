...
def aura2d_world_bridge_payload(game: GameDNA, world: GameWorldDNA) -> dict[str, Any]:
    ...
    Executable behavior parameters come only from the existing sanitized gameplay payload. Plain
    World entities are added only when Adventure or State Machine DNA references their ids, which
    lets 2D dialogue, gates, reach objectives and state machines resolve positions without exposing
    arbitrary metadata or creator source.
    """
