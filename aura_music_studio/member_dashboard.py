...
def member_dashboard(request: Request):
    ...
    security_panel = """<section class='security'><div><div class='eyebrow'>Separate security product · same account</div><h2>🛡️ Aura Sec Security Center</h2><p>Aura Sec lives inside this same signed-in Command Center account, but its protection entitlement is separate from ordinary creative membership. Open the Security Center to review Aura Sec licensing, enrolled devices, incidents and bounded action status.</p><p class='securitytruth'>The browser is a member-safe control plane only. It cannot execute endpoint commands, poll native command channels, submit native heartbeat proof, access command-signing keys or bypass strong re-authentication.</p></div><a class='btn securitybtn' href='/aura-sec'>Open Aura Sec</a></section>"""
    ...
    if esp_status in {"active", "owner"}:
        ...
        esp_panel = f"""<section class='esp'><div><div class='eyebrow'>Private Elevate Souls Productions Area</div><h2>ESP {escape(label)}</h2><p>Your approved ESP Creator/Agent permissions unlock additional areas inside this same Pulsar-Frequency House account: niche training, progress, Social Management, commercial growth and agency operations. These areas remain hidden from ordinary public members.</p></div><a class='btn espbtn' href='/command-center'>Enter ESP Hub</a></section>"""
