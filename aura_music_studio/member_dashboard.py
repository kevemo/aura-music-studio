...
def member_dashboard(request: Request):
    ...
    regular_cards = [
        (
            "🎵",
            "Music Studio",
            "Create songs, backing tracks, record performances, build around uploads, edit stems, mix and master with Aura.",
            "/studio",
            "Open Music Studio",
        ),
        (
    ...
    cards = "".join(
        f"""<article class='tool'><div class='icon'>{icon}</div><h3>{escape(title)}</h3><p>{escape(copy)}</p><a class='btn primary' href='{escape(url, quote=True)}'>{escape(cta)}</a></article>"""
        for icon, title, copy, url, cta in regular_cards
    )
