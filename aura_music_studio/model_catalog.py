...
CATALOG: tuple[ModelPolicy, ...] = (
    ...
    ModelPolicy(
        id="wan22",
        component="Wan 2.2",
        role="optional self-hosted text/image/audio-driven music-video scene generation",
        source="https://github.com/Wan-Video/Wan2.2",
        core_or_optional="optional Pro video renderer",
        code_license="Apache-2.0 upstream",
        model_license="Wan2.2 repository/model weights are published under Apache-2.0 upstream; record the exact checkpoint used",
        commercial_use="permitted subject to the upstream Apache-2.0 terms and lawful/rights-cleared member inputs",
        notes="ESP uses an owner-configured command adapter; checkpoints are never committed to the Studio repository.",
    ),
    ModelPolicy(
        id="mova",
        component="MOVA",
        role="optional synchronized video/audio and lip-sync scene generation",
        source="https://github.com/OpenMOSS/MOVA",
        core_or_optional="experimental optional Pro video renderer",
        code_license="review exact upstream revision before deployment",
        model_license="review exact MOVA checkpoint terms before deployment",
        commercial_use="not assumed until the selected code/model revision passes ESP licence review",
    ),
    ModelPolicy(
        id="ltx2",
        component="LTX-2.x",
        role="optional cinematic text/image-to-video rendering",
        source="https://github.com/Lightricks/LTX-2",
        core_or_optional="optional Pro video renderer",
        code_license="LTX-2.x Community License Agreement",
        model_license="LTX-2.x Community License Agreement; version-specific terms apply",
        commercial_use="conditional; current community terms include commercial-entity/revenue conditions, so ESP must review before activation",
        notes="Never auto-enable solely because weights are downloadable. The owner must approve the exact LTX version/licence.",
    ),
    ModelPolicy(
        id="the-muser",
        component="The Muser",
