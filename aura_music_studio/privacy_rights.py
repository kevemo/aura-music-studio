...
class PrivacyEvidenceStore:
    ...
    Requests are evidence for review, not commands to delete/export data. This keeps legal and
    identity-review decisions separate from ordinary application permissions and ESP role state.
    """
    ...
    def snapshot(self, user_id: str, *, limit: int = 50) -> dict:
        ...
        return {
            ...
            "policy_evidence": [dict(row) for row in evidence],
        }
...
def member_privacy_rights(request: Request):
    ...
    snapshot = PrivacyEvidenceStore().snapshot(user_id)
    return {
