...
def run_editor_render_job(
    ...
    *,
    ...
    The queue stores only project/sequence directives. The worker resolves all paths from the
    already-established tenant context and re-checks entitlements at execution time so a plan
    downgrade cannot leave a previously queued Pro render authorized indefinitely.
    """
...
def submit_editor_render_job(
    ...
    # Authenticate and authorize before project lookup so lower tiers cannot probe project existence.
