...
def _append_full_frame_blend(
    ...
    *,
    ...
    filters.append(f"[{suffix}baseblend]format=gbrp[{suffix}blendbase]")
    filters.append(f"[{suffix}topblend]format=gbrp[{suffix}blendtop]")
    filters.append(
...
class GroupedTrackVideoCompositor(AdvancedVideoCompositor):
    ...
    def render_video_advanced(self, sequence_id: str) -> EditorExportResult:
        ...
        try:
            ...
            next_index = 1
            for _track, entries in visual_tracks:
            ...
            result = self._record_result(
                ...
                output,
                "mp4",
                "ffmpeg-grouped-track-video-compositor",
                source_refs,
            )
