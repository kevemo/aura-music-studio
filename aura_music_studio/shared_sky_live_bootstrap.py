...
def install_shared_sky_live_community(app: Any) -> None:
    ...
    discovery remains read-only. Chat 5 may persist a server-authoritative source→profile reference
    in its own LIVE relationship state, but that reference never copies Chat 2 profile/model data,
    never grants Chat 6 entitlement and never proves or activates an executable voice processor.
    """
