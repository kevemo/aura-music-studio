...
def _browser_descriptor(value: Any) -> dict[str, Any]:
    ...
    manifest = _safe_url(source.get("manifest_url"))
    ...
    for raw in source.get("renditions") or []:
        ...
        rendition_url = _safe_url(raw.get("manifest_url") or raw.get("url"))
        if rendition_url:
    ...
    return {
        "available": available,
        "state": str(source.get("state") or ("ready" if available else "unavailable"))[:80],
        "reason": str(source.get("reason") or "")[:160],
        "manifest_url": manifest if available else None,
        "renditions": renditions,
        "captions": captions,
        "dvr": bool(source.get("dvr", False)),
        "browser_authorization_mode": str(source.get("browser_authorization_mode") or "")[:120],
...
def _interactive_state(broadcast_id: str, viewer_key: str | None) -> dict[str, Any]:
    ...
    with live.community._connect() as con:
        ...
        qa_rows = con.execute(
            """SELECT id,asker_user_id,question,state,selected,created_at,updated_at
               FROM shared_sky_qa WHERE broadcast_id=? AND state IN ('approved','answered')
...
def watch_interactive_state(broadcast_id: str, request: Request, presence_token: str | None = None):
    user_id = _access_or_raise(broadcast_id, request)
    viewer_key = f"user:{user_id}" if user_id else live.community.presence_actor(broadcast_id, presence_token)
    return _interactive_state(broadcast_id, viewer_key)


WATCH_CSS = """
:root{--bg:#061411;--panel:#0a231f;--panel2:#102f29;--line:#ffffff22;--text:#f4fff9;--muted:#b8cec7;--emerald:#5de0b2;--sardonyx:#d47f60;--gold:#f1ce79;--danger:#ff879c}
*{box-sizing:border-box}html{color-scheme:dark}body{margin:0;background:radial-gradient(circle at 5% 0,#0c6c5255,transparent 30%),radial-gradient(circle at 94% 0,#a85d4355,transparent 25%),var(--bg);color:var(--text);font-family:Inter,ui-sans-serif,system-ui,sans-serif}button,input,select{font:inherit}button,.btn{border:1px solid var(--line);background:#ffffff0a;color:var(--text);border-radius:11px;padding:9px 11px;font-weight:800;cursor:pointer}.btn{text-decoration:none;display:inline-flex;align-items:center}.primary{background:linear-gradient(115deg,var(--emerald),var(--gold));color:#032219;border:0}.wrap{width:min(1480px,calc(100% - 24px));margin:auto;padding:12px 0 44px}.top{display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap}.watch{display:grid;grid-template-columns:minmax(0,1.7fr) minmax(320px,.55fr);gap:12px;margin-top:12px}.stage{min-width:0}.player-shell{position:relative;background:#000;border:1px solid var(--line);border-radius:18px;overflow:hidden;min-height:300px;display:grid;place-items:center;outline:none}.player-shell:focus-visible{box-shadow:0 0 0 3px var(--gold)}video{width:100%;max-height:76vh;display:block;background:#000;object-fit:contain}.player-shell.portrait video{max-height:82vh;width:auto;max-width:100%}.player-status{padding:34px;max-width:720px;text-align:center}.player-status h2{margin:0 0 8px}.player-bar{display:flex;align-items:center;gap:7px;flex-wrap:wrap;padding:9px;background:#071411ee;border-top:1px solid var(--line)}.player-bar button,.player-bar select{min-height:40px}.player-bar input[type=range]{width:110px}.live-dot{display:inline-block;width:9px;height:9px;border-radius:50%;background:#e83f58;margin-right:6px}.live-dot.off{background:#788c86}.info{border:1px solid var(--line);border-radius:16px;background:var(--panel);padding:14px;margin-top:10px}.muted{color:var(--muted);line-height:1.45}.actions{display:flex;gap:7px;flex-wrap:wrap}.sidebar{border:1px solid var(--line);border-radius:18px;background:var(--panel);min-height:0;overflow:hidden;display:flex;flex-direction:column}.tabs{display:grid;grid-template-columns:repeat(3,1fr);border-bottom:1px solid var(--line)}.tabs button{border:0;border-radius:0;background:transparent}.tabs button[aria-selected=true]{background:#ffffff0d;color:var(--emerald)}.pane{display:none;padding:12px;min-height:0}.pane.active{display:flex;flex-direction:column}.chat{height:min(52vh,610px);overflow:auto;display:flex;flex-direction:column;gap:7px}.msg{padding:8px 9px;border-radius:10px;background:#ffffff08;overflow-wrap:anywhere}.msg.pinned{border:1px solid var(--gold)}.composer{display:flex;gap:7px;margin-top:8px}.composer input{flex:1;min-width:0;background:#061411;color:var(--text);border:1px solid var(--line);border-radius:10px;padding:10px}.poll,.question,.gift{padding:10px;border-radius:12px;background:#ffffff08;margin-bottom:8px}.poll-options{display:grid;gap:6px}.poll-options button{text-align:left}.selected-q{border:1px solid var(--gold)}.gift-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;max-height:280px;overflow:auto}.badge{display:inline-flex;border:1px solid var(--line);border-radius:999px;padding:4px 8px;font-size:.78rem}.sr{position:absolute;width:1px;height:1px;clip:rect(0,0,0,0);overflow:hidden;white-space:nowrap}.mobile-toggle{display:none}
@media(max-width:900px){.watch{grid-template-columns:1fr}.sidebar{max-height:55vh}.chat{height:38vh}.mobile-toggle{display:inline-flex}.player-shell{min-height:220px}}@media(max-width:520px){.wrap{width:min(100% - 12px,1480px)}.player-bar{gap:4px}.player-bar button{padding:8px}.gift-grid{grid-template-columns:1fr}.info h1{font-size:1.45rem}}@media(prefers-reduced-motion:reduce){*{scroll-behavior:auto!important;animation:none!important;transition:none!important}}
"""
...
def _gift_html(state: Any) -> str:
    ...
    for item in catalogue[:20]:
        ...
        cards.append(f"<div class='gift'><b>{name}</b><div class='muted'>{cost} Cosmic Creation Coins</div></div>")
    status = "Gift sending is enabled by the authoritative economy service." if state.get("send_enabled") else escape(str(state.get("reason") or "Gift sending is currently unavailable."))
...
def _battle_html(state: Any) -> str:
    ...
    names = ", ".join(escape(str(item.get("display_name") or item.get("name") or "Creator")) for item in participants if isinstance(item, dict))
    return f"<div class='info'><span class='badge'>Battle</span><b> {status}</b>{f'<p class=muted>{names}</p>' if names else ''}</div>"
...
def watch_page_v2(broadcast_id: str, request: Request):
    ...
    replay = _browser_descriptor(detail.get("replay")) if isinstance(detail.get("replay"), dict) else None
    descriptor = replay if broadcast.get("authoritative_state") == "ended" and replay and replay.get("available") else playback
    ...
    if manifest:
        player = f"<video id='video' controls playsinline preload='metadata' src='{escape(str(manifest), quote=True)}'>{tracks}</video><div id='playerStatus' class='player-status' hidden></div>"
    else:
        reason = escape(str((descriptor or {}).get("reason") or "playback_not_ready"))
        label = "Replay is not ready" if broadcast.get("authoritative_state") == "ended" else "LIVE playback is temporarily unavailable"
        player = f"<video id='video' controls playsinline preload='metadata' hidden>{tracks}</video><div id='playerStatus' class='player-status'><h2>{label}</h2><p class='muted' id='playerReason'>{reason}</p><button id='retryPlayback'>Retry playback</button></div>"
    ...
    live_state = str(broadcast.get("authoritative_state") or "") == "live"
    ...
    broadcast_json = json.dumps(broadcast_id)
    creator_json = json.dumps(creator_id)
    return HTMLResponse(
        ...
        <section class='watch'><div class='stage'><div id='playerShell' class='player-shell' tabindex='0' aria-label='Shared Sky video player'>{player}<div class='player-bar' id='playerBar'>
        ...
        <div class='info'><div class='top'><div><span id='liveBadge' class='badge'><span class='live-dot{' ' if live_state else ' off'}'></span>{'LIVE' if live_state else 'ENDED'}</span> <span id='viewers' class='muted'>{viewers} watching</span></div><div class='actions'><button id='follow'>{'Following' if detail.get('following') else 'Follow'}</button><button id='like'>♥ Like</button><button id='share'>Share</button><button id='report'>Report</button></div></div>
        ...
        <aside class='sidebar' id='communityPanel'><div class='tabs' role='tablist'><button role='tab' aria-selected='true' aria-controls='chatPane' id='chatTab'>Chat</button><button role='tab' aria-selected='false' aria-controls='pollPane' id='pollTab'>Polls</button><button role='tab' aria-selected='false' aria-controls='qaPane' id='qaTab'>Q&amp;A</button></div>
        <section class='pane active' id='chatPane' role='tabpanel' aria-labelledby='chatTab'><div id='chat' class='chat' role='log' aria-live='polite' aria-relevant='additions'></div><form id='chatForm' class='composer'><label class='sr' for='chatInput'>Message</label><input id='chatInput' maxlength='1000' autocomplete='off' placeholder='Message Shared Sky chat'><button>Send</button></form></section>
        <section class='pane' id='pollPane' role='tabpanel' aria-labelledby='pollTab'><div id='polls'></div></section>
        <section class='pane' id='qaPane' role='tabpanel' aria-labelledby='qaTab'><div id='questions'></div><form id='qaForm' class='composer'><label class='sr' for='qaInput'>Ask a question</label><input id='qaInput' maxlength='600' autocomplete='off' placeholder='Ask the creator'><button>Ask</button></form></section></aside></section></main>
        <script id='initialState' type='application/json'>{state_json}</script>
        <script>(()=>{{
        const id={broadcast_json},creatorId={creator_json},initial=JSON.parse(document.getElementById('initialState').textContent);let presence=null,cursor=0,retryDelay=1200,descriptor=initial.playback||{{}},ended=initial.broadcast.authoritative_state==='ended';
        ...
        function applyDescriptor(d){{descriptor=d||{{}};populateQuality(descriptor);$('jumpLive').hidden=!descriptor.dvr;const src=descriptor.available?safeMedia(descriptor.manifest_url):'';if(!src){{setStatus(ended?'Replay is not ready':'LIVE playback is temporarily unavailable',descriptor.reason);return}}const v=$('video'),s=$('playerStatus');s.hidden=true;v.hidden=false;if(v.src!==src){{v.src=src;v.load()}}retryDelay=1200}}
        ...
        async function refreshState(){{try{{const d=await req(`/shared-sky/live/api/watch/${{encodeURIComponent(id)}}`);ended=d.broadcast.authoritative_state==='ended';$('liveBadge').textContent=ended?'ENDED':'LIVE';$('viewers').textContent=(d.viewer_count||0)+' watching';if(ended&&d.replay&&d.replay.available)applyDescriptor(d.replay);else if(!ended)await refreshPlayback()}}catch(e){{announce(e.message)}}}}
        const v=$('video'),shell=$('playerShell');v.addEventListener('loadedmetadata',()=>{{shell.classList.toggle('portrait',v.videoHeight>v.videoWidth);$('playToggle').textContent=v.paused?'Play':'Pause'}});v.addEventListener('play',()=>$('playToggle').textContent='Pause');v.addEventListener('pause',()=>$('playToggle').textContent='Play');v.addEventListener('error',()=>{{setTimeout(refreshPlayback,retryDelay);retryDelay=Math.min(12000,retryDelay*1.8)}});
        ...
        async function refreshInteractives(){{try{{const token=presence?.resume_token||'';const d=await req(`/shared-sky/live/api/watch/${{encodeURIComponent(id)}}/interactives?presence_token=${{encodeURIComponent(token)}}`);renderPolls(d);renderQuestions(d)}}catch(e){{}}}}
        ...
        $('chatForm').onsubmit=async e=>{{e.preventDefault();const x=$('chatInput'),value=x.value.trim();if(!value)return;try{{const m=await req(`/shared-sky/live/api/watch/${{encodeURIComponent(id)}}/chat`,{{method:'POST',body:JSON.stringify({{message:value,idempotency_key:crypto.randomUUID()}})}});x.value='';addMsg(m)}}catch(err){{announce(err.message)}}}};
        $('qaForm').onsubmit=async e=>{{e.preventDefault();const x=$('qaInput'),value=x.value.trim();if(!value)return;try{{await req(`/shared-sky/live/api/watch/${{encodeURIComponent(id)}}/qa`,{{method:'POST',body:JSON.stringify({{question:value,idempotency_key:crypto.randomUUID()}})}});x.value='';announce('Question submitted for LIVE Q&A review.')}}catch(err){{announce(err.message)}}}};
...
__all__ = ["router", "watch_page_v2", "watch_interactive_state", "_browser_descriptor"]
