...
def update_profile(user_id: str, session_id: str, body: ProfilePatch) -> dict[str, Any]:
    ...
    if current["transition_state"] != "idle":
        raise StudioConflict("Production profile changes are locked during a transition")
    return studio_repo._mutate(  # Same repository/version transaction; no parallel profile store.
        user_id,
        session_id,
...
def create_studio_source(
    user_id: str,
    session_id: str,
    body: StudioSourceCreate,
) -> dict[str, Any]:
    current = studio_repo.get_session(user_id, session_id)
    if current["transition_state"] != "idle":
    ...
    if body.source_type in {"camera", "microphone", "screen"}:
        ...
        # Browser MediaStreams are process-local and cannot be truthfully persisted as attached.
...
def patch_source_z(
    ...
    updated = shared_sky.update_source(
        user_id,
        source_id,
        SourceUpdate(z_index=body.z_index),
    )
    session = studio_repo.set_autosave_state(
        user_id,
        session_id,
...
.app{height:100vh;display:grid;grid-template-columns:230px 1fr 300px;grid-template-rows:58px minmax(0,1fr) 220px;gap:8px;padding:8px}
...
.source{position:absolute;overflow:hidden;background:#15303e;border:1px solid transparent;user-select:none;touch-action:none}.source.selected{border-color:var(--aqua);box-shadow:0 0 0 1px var(--aqua)}.source.locked{border-style:dashed}.source video,.source img{width:100%;height:100%;object-fit:cover}.placeholder{width:100%;height:100%;display:grid;place-items:center;text-align:center;color:var(--muted);padding:6px}.resize{position:absolute;width:14px;height:14px;right:-1px;bottom:-1px;background:var(--aqua);border:2px solid #041016;cursor:nwse-resize;z-index:4}
.scene,.source-row{display:block;width:100%;text-align:left;background:#ffffff08;border:1px solid var(--line);color:var(--text);padding:8px;border-radius:9px;margin:5px 0}.scene.active,.source-row.active{border-color:var(--aqua)}button,input,select{font:inherit}button,.btn{background:#102d39;border:1px solid var(--line);color:var(--text);border-radius:8px;padding:7px 10px;font-weight:750}.cut{background:var(--amber);color:#1a1200}.take{background:var(--red);color:#190108}.row{display:flex;gap:6px;align-items:center;flex-wrap:wrap}.muted{color:var(--muted);font-size:.8rem}.status{font-size:.78rem}.field{display:grid;grid-template-columns:72px 1fr;gap:7px;align-items:center;margin:6px 0}.field input,.field select{width:100%;background:#041016;border:1px solid var(--line);color:var(--text);padding:6px;border-radius:7px}
.mixer{display:flex;gap:8px;overflow:auto}.channel{min-width:155px;border:1px solid var(--line);border-radius:9px;padding:8px}.meter{height:7px;background:#0008;border-radius:5px;overflow:hidden;margin:6px 0}.meter>i{display:block;width:0;height:100%;background:currentColor}.offline{opacity:.55}.danger{color:#ff9cab}:focus-visible{outline:3px solid var(--aqua);outline-offset:2px}
@media(max-width:1050px){.app{grid-template-columns:190px 1fr;grid-template-rows:58px minmax(0,1fr) auto 220px}.right{grid-column:1/-1;grid-row:3}.bottom{grid-row:4}.monitors{grid-template-columns:1fr}}
...
const initialProfile=qs.get('profile_key')||'landscape-1080';
const state={session:null,project:null,transport:null,selected:new Set(),streams:new Map(),zoom:1};
...
function preview(){return state.project?.scenes?.find(s=>s.id===state.session?.preview_scene_id);}
function sourceById(id){return preview()?.sources?.find(s=>s.id===id);}
function transformOf(src){return src.config?.transform||{x:0,y:0,width:1,height:1,rotation:0,opacity:1};}
function sourceStyle(src){const x=transformOf(src);return `left:${x.x*100}%;top:${x.y*100}%;width:${x.width*100}%;height:${x.height*100}%;opacity:${x.opacity??1};transform:rotate(${x.rotation||0}deg);z-index:${src.z_index||0}`;}
...
function sourceLayer(src,programme=false){if(!src.visible)return'';const selected=!programme&&state.selected.has(src.id);return `<div class='source ${selected?'selected':''} ${src.locked?'locked':''}' data-source='${esc(src.id)}' style='${sourceStyle(src)}'>${sourceMedia(src)}${!programme&&!src.locked?'<span class=resize data-resize=1 aria-hidden=true></span>':''}</div>`;}
...
function render(){if(!state.session||!state.project)return;$('#version').textContent=`v${state.session.version}`;$('#transport').textContent=`Transport ${state.transport?.state||'unknown'}`;$('#profile').value=state.session.profile_key;$('#scenes').innerHTML=state.project.scenes.map(s=>`<button class='scene ${s.id===state.session.preview_scene_id?'active':''}' data-scene='${s.id}'>${esc(s.name)}</button>`).join('');const p=preview();$('#previewCanvas').innerHTML=previewChrome()+(p?.sources||[]).map(s=>sourceLayer(s)).join('');const prog=state.session.programme_snapshot||{};$('#programmeCanvas').innerHTML=(prog.sources||[]).map(s=>sourceLayer(s,true)).join('')||'<div class=placeholder>OFF AIR / no committed Programme snapshot</div>';$('#sources').innerHTML=(p?.sources||[]).slice().sort((a,b)=>(b.z_index||0)-(a.z_index||0)).map(s=>`<button class='source-row ${state.selected.has(s.id)?'active':''}' data-source-row='${s.id}'>${s.locked?'🔒 ':''}${esc(s.name)} <small>${esc(s.source_type)}</small></button>`).join('');sizeCanvases();bindCanvas();attachStreams();renderInspector();renderMixer();}
...
async function selectScene(id){try{assign(await api(`/shared-sky/studio/api/sessions/${state.session.id}/preview`,{method:'POST',body:JSON.stringify({scene_id:id,expected_version:state.session.version})}));}catch(e){await handle(e);}}
function snap(v){for(const target of [0,.5,1])if(Math.abs(v-target)<.012)return target;return Math.max(-4,Math.min(4,v));}
...
function pointerStart(e){const id=e.currentTarget.dataset.source,src=sourceById(id);if(!src||src.locked)return;if(!state.selected.has(id)){state.selected.clear();state.selected.add(id);render();const replacement=$(`#previewCanvas [data-source='${CSS.escape(id)}']`);if(replacement){replacement.dispatchEvent(new PointerEvent('pointerdown',{pointerId:e.pointerId,clientX:e.clientX,clientY:e.clientY,bubbles:true}));}return;}const target=e.currentTarget,mode=e.target.dataset.resize?'resize':'move',base=new Map([...state.selected].map(sid=>[sid,{...transformOf(sourceById(sid))}])),rect=$('#previewCanvas').getBoundingClientRect(),sx=e.clientX,sy=e.clientY;target.setPointerCapture(e.pointerId);function move(ev){const dx=(ev.clientX-sx)/rect.width,dy=(ev.clientY-sy)/rect.height;for(const sid of state.selected){const s=sourceById(sid),b=base.get(sid);if(!s||s.locked)continue;const next={...b};if(mode==='move'){next.x=snap(b.x+dx);next.y=snap(b.y+dy);}else{next.width=Math.max(.02,snap(b.width+dx));next.height=Math.max(.02,snap(b.height+dy));}s.config={...(s.config||{}),transform:next};}updateSourceNodes();}async function up(){target.removeEventListener('pointermove',move);target.removeEventListener('pointerup',up);await persistSelected();render();}target.addEventListener('pointermove',move);target.addEventListener('pointerup',up);}
async function persistSelected(){for(const id of [...state.selected]){const src=sourceById(id);if(!src||src.locked)continue;try{const d=await api(`/shared-sky/studio/api/sessions/${state.session.id}/sources/${id}/transform`,{method:'PATCH',body:JSON.stringify({expected_session_version:state.session.version,transform:transformOf(src)})});state.session=d.session;}catch(e){await handle(e);break;}}await refreshProject();}
async function refreshProject(){if(!state.session)return;assign(await api(`/shared-sky/studio/api/sessions/${state.session.id}`));}
...
async function addSource(type){let stream=null;try{stream=await captureFor(type);const name=type[0].toUpperCase()+type.slice(1),result=await api(`/shared-sky/studio/api/sessions/${state.session.id}/sources`,{method:'POST',body:JSON.stringify({source_type:type,name,visible:true,config:{privacy:'programme_safe'}})});state.session=result.session;if(stream){state.streams.set(result.source.id,stream);watchStream(result.source.id,name,stream);}await refreshProject();}catch(e){stream?.getTracks().forEach(t=>t.stop());handle(e);}}
function watchStream(id,name,stream){for(const track of stream.getTracks())track.addEventListener('ended',()=>{state.streams.delete(id);$('#notice').textContent=`${name} disconnected or permission ended`;render();},{once:true});}
async function deleteSelected(){for(const id of [...state.selected]){try{const r=await api(`/shared-sky/studio/api/sessions/${state.session.id}/sources/${id}`,{method:'DELETE',body:JSON.stringify({expected_session_version:state.session.version})});state.session=r.session;state.streams.get(id)?.getTracks().forEach(t=>t.stop());state.streams.delete(id);}catch(e){await handle(e);break;}}state.selected.clear();await refreshProject();}
...
$('#cut').onclick=()=>take('cut');$('#take').onclick=()=>take('transition');$('#profile').onchange=setProfile;$('#zoom').oninput=e=>{state.zoom=Number(e.target.value);sizeCanvases();};$('#addCamera').onclick=()=>addSource('camera');$('#addMic').onclick=()=>addSource('microphone');$('#addScreen').onclick=()=>addSource('screen');$('#addText').onclick=()=>addSource('text');$('#deleteSource').onclick=deleteSelected;
...
<section class='center'><div class='monitors'><section class='monitor preview' aria-label='Preview monitor'><span class='label'>PREVIEW</span><div class='viewport'><div id='previewCanvas' class='canvas'></div></div></section><section class='monitor programme' aria-label='Programme monitor'><span class='label'>PROGRAMME</span><div class='viewport'><div id='programmeCanvas' class='canvas'></div></div></section></div><div class='top'><label>Transition <select id='transition'><option>fade</option><option>dip_to_colour</option><option>slide</option><option>push</option><option>zoom</option></select></label><label>ms <input id='duration' type='number' min='0' max='20000' value='350' style='width:88px'></label><button id='cut' class='cut'>CUT</button><button id='take' class='take'>TRANSITION</button><span class='muted'>Alt+C CUT · Ctrl+Enter transition · arrows nudge · Shift+arrows coarse nudge</span></div></section>
...
@router.get(
    "/shared-sky/studio/professional",
