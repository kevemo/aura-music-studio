...
function countdownValue(ms,showDays){if(ms<=0)return null;let total=Math.floor(ms/1000),days=Math.floor(total/86400);total%=86400;const h=Math.floor(total/3600);total%=3600;const m=Math.floor(total/60),s=total%60;const hh=String(h).padStart(2,'0'),mm=String(m).padStart(2,'0'),ss=String(s).padStart(2,'0');return showDays&&days?`${days}d ${hh}:${mm}:${ss}`:`${String(h+days*24).padStart(2,'0')}:${mm}:${ss}`;}
...
def install_professional_motion_graphics_ui(app: Any) -> None:
    ...
    setattr(wrapped, "_shared_sky_motion_graphics_ui", True)
