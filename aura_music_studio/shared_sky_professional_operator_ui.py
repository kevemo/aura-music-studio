...
async function createOperatorProfileUI(){const name=prompt('Operator profile name','Studio Operator');if(!name)return;const shortcut=prompt('First hotkey (example CTRL+SHIFT+K). Leave blank for none.','');let command='undo';const hotkeys={};if(shortcut){command=prompt('Command: cut, transition, undo, redo, scene_next, scene_previous, marker_highlight','undo')||'undo';hotkeys[shortcut]=command;}try{await api(`/shared-sky/studio/api/projects/${encodeURIComponent(projectId)}/operator-profiles`,{method:'POST',body:JSON.stringify({name,hotkeys,macros:[],activate:true})});await loadOperatorProfiles();$('#notice').textContent='Operator profile created and activated.';}catch(e){handle(e);}}
...
$('#activateOperatorProfile').onclick=activateOperatorProfileUI;$('#createOperatorProfile').onclick=createOperatorProfileUI;
...
def install_professional_operator_ui(app: Any) -> None:
    ...
    if getattr(current, "_shared_sky_operator_ui", False):
        return
