...
<section id='operatorConsole' class='operator-console' aria-label='Operator hotkey and macro profile'>
  ...
  <div class='operator-profile-row'><select id='operatorProfile' aria-label='Active operator profile'><option value=''>Default fixed controls</option></select><button id='activateOperatorProfile'>Activate</button><button id='createOperatorProfile'>New</button></div>
  <div class='operator-profile-row'><button id='addOperatorHotkey'>Add hotkey</button><button id='addOperatorMacro'>Add macro</button><button id='deleteOperatorProfile' class='danger'>Delete profile</button></div>
  <div id='operatorHotkeys' class='hotkey-grid'></div>
  <div id='operatorMacros' class='macro-buttons'></div>
  <div class='operator-warning'>Macros are explicit operator actions. Transport, recording, participant and destination mutations are not valid macro commands. Programme-changing macros confirm every run.</div>
</section>
"""
...
function renderOperatorProfile(){const select=$('#operatorProfile');if(!select)return;select.innerHTML=`<option value=''>Default fixed controls</option>`+operatorUI.profiles.map(p=>`<option value='${esc(p.id)}' ${p.is_active?'selected':''}>${esc(p.name)}${p.is_active?' · active':''}</option>`).join('');const profile=activeOperatorProfile();$('#operatorHotkeys').innerHTML=profile?Object.entries(profile.hotkeys||{}).map(([key,command])=>`<code>${esc(key)}</code><span>${esc(command.replaceAll('_',' '))}</span>`).join(''):'<span class=muted>No custom hotkeys active.</span>';$('#operatorMacros').innerHTML=profile?(profile.macros||[]).map((m,i)=>`<button data-operator-macro='${i}'>${esc(m.name)}</button>`).join(''):' ';$$('[data-operator-macro]').forEach(b=>b.onclick=()=>runOperatorMacro(Number(b.dataset.operatorMacro)));for(const id of ['addOperatorHotkey','addOperatorMacro','deleteOperatorProfile'])if($(`#${id}`))$(`#${id}`).disabled=!profile;}
...
async function activateOperatorProfileUI(){const id=$('#operatorProfile').value;if(!id){operatorUI.active=null;renderOperatorProfile();$('#notice').textContent='Using fixed default Studio controls for this browser session.';return;}operatorUI.busy=true;try{const d=await api(`/shared-sky/studio/api/projects/${encodeURIComponent(projectId)}/operator-profiles/${encodeURIComponent(id)}/activate`,{method:'POST'});await loadOperatorProfiles();$('#notice').textContent=`Operator profile “${d.profile?.name||'profile'}” activated.`;}catch(e){handle(e);}finally{operatorUI.busy=false;}}
...
async function deleteOperatorProfileUI(){const profile=activeOperatorProfile();if(!profile)return;if(!confirm(`Delete operator profile “${profile.name}”? Active profiles must be replaced before deletion.`))return;try{await api(`/shared-sky/studio/api/projects/${encodeURIComponent(projectId)}/operator-profiles/${encodeURIComponent(profile.id)}`,{method:'DELETE'});await loadOperatorProfiles();$('#notice').textContent='Operator profile deleted.';}catch(e){handle(e);}}
...
$('#activateOperatorProfile').onclick=activateOperatorProfileUI;$('#createOperatorProfile').onclick=createOperatorProfileUI;$('#addOperatorHotkey').onclick=addOperatorHotkeyUI;$('#addOperatorMacro').onclick=addOperatorMacroUI;$('#deleteOperatorProfile').onclick=deleteOperatorProfileUI;
