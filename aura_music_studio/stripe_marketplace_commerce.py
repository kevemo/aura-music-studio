...
def _refund_targets_verified_marketplace(
    ...
    The fast path uses already-persisted fee evidence. If a refund event arrives before the
    ...
    marketplace order before the event is admitted to marketplace settlement handling.
    """
...
def process_verified_marketplace_stripe_event(
    *,
    ...
    Event metadata can identify the opaque local order only. Price, tenant, publication, creator
    payee and owner provenance are re-established from immutable local order data plus canonical
    Stripe provider evidence before any marketplace settlement or reversal mutation.
    """
    if event_type in {"checkout.session.completed", "checkout.session.async_payment_succeeded"}:
