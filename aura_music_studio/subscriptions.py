...
class SubscriptionLedger:
    """Tracks paid membership periods independently of the payment provider.
    ...
    def verify_payment(self, user_id: str, plan_id: str, payment_reference: str, *, period_days: int = 31) -> dict:
        ...
        if user.get("status") not in {"approved_pending_payment", "active"}:
            raise ValueError("ESP approval is required before payment can activate a membership")
    ...
    def enforce(self, user: dict) -> dict:
        ...
        with self._connect() as con:
            ...
            con.execute(
                "UPDATE users SET status='approved_pending_payment', billing_status='past_due' WHERE id=?",
                (user["id"],),
            )
