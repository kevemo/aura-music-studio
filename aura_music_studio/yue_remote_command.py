...
def _safe_provider_url(base: str, value: str) -> str:
    """Resolve a provider-returned URL without allowing it to pivot off-provider.

    YuE may be self-hosted on a private network, so the configured provider origin itself is
    an operator trust boundary. A provider response is not allowed to turn that configured
    access into a general-purpose server-side URL fetcher.
    """
