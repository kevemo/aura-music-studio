...
# Registry authentication is deliberately external to this script. Credentials belong in the
...
# Scan exactly what was pushed. HIGH/CRITICAL findings block release by default for both the
# Command Center and the required music renderer image.
trivy image --exit-code 1 --severity HIGH,CRITICAL --scanners vuln "$IMMUTABLE_COMMAND_IMAGE"
trivy image --exit-code 1 --severity HIGH,CRITICAL --scanners vuln "$IMMUTABLE_ACESTEP_IMAGE"
...
cosign verify --key "$COSIGN_VERIFY_KEY" \
  -a "git_sha=$GIT_SHA" \
  "$IMMUTABLE_COMMAND_IMAGE" >/dev/null
cosign verify --key "$COSIGN_VERIFY_KEY" \
  -a "component=ace-step" \
  -a "upstream_commit=$ACESTEP_UPSTREAM_COMMIT" \
  "$IMMUTABLE_ACESTEP_IMAGE" >/dev/null

python3 - "$OUTPUT_RELEASE" "$GIT_SHA" "$IMMUTABLE_COMMAND_IMAGE" "$IMMUTABLE_ACESTEP_IMAGE" "$ACESTEP_UPSTREAM_COMMIT" <<'PY'
import json, os, sys
path, sha, command_image, ace_step_image, ace_step_commit = sys.argv[1:]
payload = {
    "schema_version": 1,
    "git_sha": sha,
    "command_center_image": command_image,
    "ace_step_image": ace_step_image,
    ...
    "supply_chain": {
        ...
        "ace_step_trivy_high_critical_gate": True,
        "ace_step_cosign_signature_verified": True,
    },
    "notes": "Command Center and ACE-Step scanner/signature gates passed. Owner/release approval is still required before deployment."
}
...
echo "Release candidate built, scanned and signed: $IMMUTABLE_COMMAND_IMAGE"
echo "ACE-Step release image built, scanned and signed: $IMMUTABLE_ACESTEP_IMAGE"
