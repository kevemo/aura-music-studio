# Aura Core 0.19 — ChatGPT-style co-creator and Studio operating intelligence
...
Aura Core is the conversational operating layer of Pulsar-Frequency House. It is intentionally broader than a music chatbot: members can use it as a general assistant while also pinning a private Studio project and asking Aura to inspect, plan or execute connected creative operations.

The design goal is **fast, truthful, private, editable assistance**. Aura should feel like one continuous AI workspace while the underlying music, image, video, voice, web and ESP systems remain permissioned modules.

## 1. ChatGPT-style workspace features connected in 0.19

The `/aura-intelligence` workspace now provides:
...
- explicit Aura Memory management;
- file attachments;
...
- Aura speech output through the configured local TTS path;
...
This is a **ChatGPT-style product experience**, not a claim that Pulsar-Frequency House copies proprietary OpenAI implementation details or has feature-for-feature parity with every version of ChatGPT.

## 2. Fast-path reasoning

General conversation should not incur an unnecessary planning round trip.

Aura Core therefore uses three routing layers:
...
Aura Core is offline/local-first.
...
No cloud model is silently assumed. If no reasoning model is reachable, Aura returns an explicit availability error. If a Studio tool has already completed but the conversational model then fails, the durable tool record is preserved.

## 4. Project-aware conversation

A conversation can pin one member-owned project. Aura then receives a compact private snapshot rather than requiring the user to restate the project on every turn.
...
Long-term Aura Memory is **explicit**.
...
- the Aura Memory panel.

Saved memories are visible and deletable by the member.

Thread message history is a separate concept: it is stored because it is the conversation itself, not because Aura has decided to create a profile about the user.

## 6. Attachments and multimodal context

### Documents

Aura extracts bounded text from supported text/code files, PDFs, DOCX and XLSX. Extracted text is stored with the private attachment record so repeated turns do not have to parse the file again.

### Images

If `AURA_VISION_MODEL` and the local Ollama endpoint are configured, Aura can analyse an uploaded image locally. The resulting textual visual analysis is saved with the private attachment and injected into subsequent reasoning.
...
Aura can sample chronological frames with FFmpeg and submit those frames to the local vision model. This is explicitly labelled **sampled-frame analysis**, not frame-perfect video inspection.
...
Aura web access reuses the existing protected web gateway rather than exposing unrestricted server-side HTTP requests.
...
## 8. Studio tools currently exposed to Aura Core
...
If ComfyUI is not configured, Aura stores the requested directive but reports `renderer_configured=false`. It must never claim that an image or video was generated when no renderer accepted the job.
...
Aura Core can inspect Voice Profiles, but voice cloning/conversion itself remains governed by Voice House and the project rights ledger.
...
Aura Core is available to ordinary Pulsar-Frequency House members, but it does **not** make ESP Creator Network data public to subscribers.
...
A future ESP-specific Aura tool must perform the same permission check as the ESP route it represents; it must never rely only on the language model deciding that a user looks like an agent.

## 13. Truthfulness contract

Aura must not say an operation succeeded unless the corresponding durable tool result says it succeeded.
...
Aura Core 0.19 establishes the assistant runtime. The next layers should extend the existing architecture rather than create another chatbot:
...
9. optional 3D/avatar presentation for Aura as a UI layer, without coupling the avatar to reasoning/tool correctness.

The critical architectural rule remains: **one Aura conversation, many permissioned tools, durable editable project state, no fabricated execution.**
