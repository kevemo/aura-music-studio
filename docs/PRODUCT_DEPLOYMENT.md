...
- Free / Tier 2 / Unlimited Pro entitlement enforcement;
...
The public membership tiers are defined server-side in `aura_music_studio/plans.py`.
...
Highest normal enabled creative access and effectively unlimited ordinary use, subject to fair-use, infrastructure, provider-capacity, rate-control, anti-abuse, safety, rights and legal safeguards. Eligible publishing remains subject to marketplace, entitlement, rights and accounting gates.
...
The current codebase contains hardened Stripe subscription/Coin/marketplace payment paths plus compatibility payment surfaces retained from earlier architecture where still required.
...
- marketplace checkout is bound to an immutable local order and buyer;
- marketplace settlement requires verified provider evidence rather than redirect state;
...
A successful Vercel build alone is not full product production evidence. The release candidate still needs the complete provider/data/security/operations gates described below.
