...
def test_duplicate_node_ids_fail_closed():
    with pytest.raises(ValueError, match="Duplicate effect node id"):
        make_effect_system(
            ...
            "Duplicate Nodes",
            (
                EffectNodeSpec("same", "music.fx.gain"),
                EffectNodeSpec("same", "music.fx.reverb"),
            ),
        )
...
def test_mix_must_be_bounded():
    with pytest.raises(ValueError, match="mix must be between 0 and 1"):
        make_effect_system(
            "bad.mix",
            "Bad Mix",
            (EffectNodeSpec("gain", "music.fx.gain", mix=1.5),),
        )
...
def test_fingerprint_is_deterministic_for_same_effective_graph():
    payload = {
        ...
        "nodes": [
            ...
            {
                "id": "width",
                "catalogue_item_id": "music.fx.stereo_width",
...
def test_maximum_node_guard_is_enforced():
    nodes = tuple(
        EffectNodeSpec(f"gain-{index}", "music.fx.gain", {"db": 0})
        for index in range(MAX_EFFECT_NODES + 1)
    )
    ...
    result = compile_effect_system_payload(
        {
            "id": "public.safe",
            "name": "Public Safe",
            "nodes": [{"id": "limiter", "catalogue_item_id": "music.fx.limiter"}],
        }
    )
