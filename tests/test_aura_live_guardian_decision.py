...
def test_critical_threat_cannot_be_suppressed_by_creator_policy():
    result = decide_with_creator_policy(
        authorization=_authorization(),
        capabilities=_capabilities(),
...
def test_policy_never_bypasses_advisory_mode():
    result = decide_with_creator_policy(
        ...
        capabilities=_capabilities(),
        signal=ModerationSignal(category="spam", confidence=0.9, severity=2),
        policy=_policy(),
    )
