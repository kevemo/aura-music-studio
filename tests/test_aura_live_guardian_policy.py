...
def test_creator_policy_deduplicates_phrases_and_matches_case_insensitively(tmp_path):
    ...
    policy = store.save(
        user_id="creator-1",
        blocked_phrases=["no drama", "  NO   DRAMA ", "spam link"],
...
def test_unknown_category_and_invalid_sensitivity_fail_closed(tmp_path):
    ...
    with pytest.raises(ValueError, match="unknown moderation category"):
        store.save(
            user_id="creator-1",
            blocked_phrases=[],
            enabled_categories={"made_up_category"},
            actor="member:creator-1",
        )
    with pytest.raises(ValueError, match="spam sensitivity"):
        store.save(
            user_id="creator-1",
            blocked_phrases=[],
            spam_sensitivity="extreme",  # type: ignore[arg-type]
            actor="member:creator-1",
        )
...
def test_phrase_limits_are_bounded(tmp_path):
    ...
    with pytest.raises(ValueError, match="too long"):
        store.save(
            user_id="creator-1",
            blocked_phrases=["x" * 121],
            actor="member:creator-1",
        )
    with pytest.raises(ValueError, match="too many"):
        store.save(
            user_id="creator-1",
            blocked_phrases=[f"phrase {index}" for index in range(101)],
            actor="member:creator-1",
        )
