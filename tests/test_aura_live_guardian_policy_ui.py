...
from aura_music_studio.aura_live_guardian_policy_ui import (
    guardian_policy_page,
    router,
    save_guardian_policy,
)
...
def test_policy_page_locks_critical_categories(monkeypatch, tmp_path):
    ...
    assert "does not create TikTok API authority" in body
...
def test_creator_policy_save_persists_preferences_and_forces_critical_protection(monkeypatch, tmp_path):
    ...
    response = save_guardian_policy(
        RequestStub(_member()),
        blocked_phrases="no drama\nspam me",
        language_tolerance="relaxed",
