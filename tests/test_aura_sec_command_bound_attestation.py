...
def test_same_command_id_with_different_signed_payload_is_rejected_before_native(tmp_path):
    ...
    # The rejected mismatched command must not consume the underlying one-time challenge.
...
def test_tampering_base_assertion_after_binding_is_still_rejected_fail_closed(tmp_path):
    ...
    # Binding covers the base assertion payload digest; the existing attestation verifier then
    # independently recomputes that payload digest from all base fields before challenge consume.
