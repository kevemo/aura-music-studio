...
def test_production_app_mounts_member_safe_aura_sec_security_center():
    html_route = _route("/system/aura-sec")
    overview_route = _route("/system/aura-sec/overview")

    assert html_route.methods == {"GET"}
    assert overview_route.methods == {"GET"}
    assert "/system/aura-sec/overview" in app.openapi()["paths"]
...
def test_aura_sec_browser_surface_does_not_mount_native_authority_routes():
    ...
    forbidden = {
        "/system/aura-sec/native/poll",
        "/system/aura-sec/native/heartbeat",
        "/system/aura-sec/native/receipt",
        "/system/aura-sec/native/execute",
        "/system/aura-sec/sign",
        "/system/aura-sec/approve",
        "/system/aura-sec/actions/execute",
    }
    assert forbidden.isdisjoint(paths)

    for route in app.routes:
        if not isinstance(route, APIRoute) or not route.path.startswith("/system/aura-sec"):
            continue
        assert (route.methods or set()) <= {"GET", "HEAD"}


def test_aura_sec_overview_is_not_a_public_anonymous_health_endpoint():
    route = _route("/system/aura-sec/overview")
