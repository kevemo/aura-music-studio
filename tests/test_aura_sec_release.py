...
def test_verified_manifest_is_downloadable_only_after_signature_callback_succeeds():
    ...
    result = validate_release_manifest(_manifest(), trusted_key_ids={TRUSTED_KEY}, signature_verifier=verifier, now=NOW)
...
def test_legacy_manifest_schema_without_signed_sequence_is_rejected():
    with pytest.raises(ReleaseManifestError, match="signed sequence metadata is required"):
        validate_release_manifest(_manifest(schema_version=1), trusted_key_ids={TRUSTED_KEY}, signature_verifier=_valid_verifier, now=NOW)


def test_valid_old_signed_release_is_rejected_as_rollback():
    with pytest.raises(ReleaseManifestError, match="rollback rejected"):
        validate_release_manifest(_manifest(release_sequence=41), trusted_key_ids={TRUSTED_KEY}, signature_verifier=_valid_verifier, now=NOW, minimum_release_sequence=42, trusted_sequence_sha256="a" * 64)


def test_same_sequence_can_only_redownload_identical_trusted_artifact():
    result = validate_release_manifest(_manifest(release_sequence=42), trusted_key_ids={TRUSTED_KEY}, signature_verifier=_valid_verifier, now=NOW, minimum_release_sequence=42, trusted_sequence_sha256="a" * 64)
    assert result["downloadable"] is True
    with pytest.raises(ReleaseManifestError, match="equivocation rejected"):
        validate_release_manifest(_manifest(release_sequence=42, artifact_sha256="b" * 64), trusted_key_ids={TRUSTED_KEY}, signature_verifier=_valid_verifier, now=NOW, minimum_release_sequence=42, trusted_sequence_sha256="a" * 64)


def test_bad_signature_fails_closed():
    with pytest.raises(ReleaseManifestError, match="signature is invalid"):
        validate_release_manifest(_manifest(), trusted_key_ids={TRUSTED_KEY}, signature_verifier=lambda *_args: False, now=NOW)


def test_untrusted_signing_identity_is_rejected_before_download():
    with pytest.raises(ReleaseManifestError, match="not trusted"):
        validate_release_manifest(_manifest(), trusted_key_ids={"another-release-key"}, signature_verifier=lambda *_args: True, now=NOW)
...
def test_artifact_and_evidence_urls_must_be_https():
    with pytest.raises(ReleaseManifestError, match="artifact_url must be an absolute HTTPS URL"):
        validate_release_manifest(_manifest(artifact_url="http://downloads.example.test/aura.exe"), trusted_key_ids={TRUSTED_KEY}, signature_verifier=lambda *_args: True, now=NOW)
