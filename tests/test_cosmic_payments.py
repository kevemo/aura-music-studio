...
class FakeSignedProvider:
    ...
    def create_checkout(self, *, purchase_id, user_id, fiat_amount_minor, fiat_currency, coin_quantity):
        assert isinstance(fiat_amount_minor, int)
        assert isinstance(coin_quantity, int)
        return CoinCheckout(self.name, f"pay-{purchase_id}", f"https://example.invalid/checkout/{purchase_id}")
...
def test_fake_provider_checkout_uses_server_minor_units_and_coin_quantity():
    registry = CoinPaymentProviderRegistry(); registry.register(FakeSignedProvider())
    checkout = registry.get("fake-signed").create_checkout(
        purchase_id="purchase-1", user_id="viewer-1", fiat_amount_minor=500,
        fiat_currency="GBP", coin_quantity=1000,
    )
...
def test_fake_provider_rejects_forged_webhook_and_accepts_verified_one():
    ...
    assert event.verified is True and event.event_type == "confirmed"
