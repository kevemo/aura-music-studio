...
from aura_music_studio.creative_effect_entitlements import (
...
def test_admin_refund_restores_coins_and_revokes_effect_atomically(tmp_path):
    ...
    refund = effects.refund_and_revoke(
        "member-a",
        "music.fx.limiter",
        reference="refund:1",
...
def test_refunded_effect_can_be_purchased_again_with_new_idempotency_key(tmp_path):
    ...
    effects.refund_and_revoke(
        "member-a", "music.fx.limiter", reference="refund:1", reason="refund", actor="admin"
    )
