...
def test_payment_options_are_currency_explicit_and_keep_legacy_alias():
    ...
    pro_options = [item for item in public if item["plan_id"] == "pro"]
    assert [(item["billing_period"], item["amount_minor"]) for item in pro_options] == [
        ("month", 999),
        ("year", 9900),
    ]
    ...
    assert instructions["billing_period"] == "month"
    ...
    annual = payment_instructions("pro", "year")
    assert annual["currency"] == "GBP"
    assert annual["billing_period"] == "year"
    assert annual["amount"] == "99.00"
    assert annual["amount_minor"] == 9900
    assert annual["display_amount"] == "£99.00"
