...
def test_sqlite_rate_store_is_shared_between_instances_and_does_not_store_raw_client(tmp_path: Path):
    ...
    try:
        ...
        con.close()
    assert rows
    assert all(row[0] != "203.0.113.41" for row in rows)
    assert {row[1] for row in rows} == {"/auth/login"}


def test_sliding_window_expiry_reopens_admission(tmp_path: Path):
...
def test_production_middleware_shares_attempt_budget_across_app_instances(tmp_path: Path, monkeypatch):
...
def test_development_keeps_non_durable_fallback_behavior(tmp_path: Path, monkeypatch):
    db = tmp_path / "should-not-be-created.sqlite3"
