...
def test_basic_cover_is_denied_before_project_lookup(monkeypatch):
    ...
    body = EngineeringJobRequest(
        operation="cover",
        asset_id="asset-1",
        transform_prompt="Turn this into a cinematic rock arrangement",
    )
...
def test_basic_repaint_is_denied_before_project_lookup(monkeypatch):
    ...
    body = EngineeringJobRequest(
        operation="repaint",
        asset_id="asset-1",
        transform_prompt="Replace this section with a stronger chorus",
        repaint_start=20,
        repaint_end=40,
    )
...
def test_repaint_requires_bounded_forward_region():
    with pytest.raises(ValidationError):
        EngineeringJobRequest(
            operation="repaint",
            asset_id="asset-1",
            transform_prompt="Regenerate this phrase",
            repaint_start=30,
            repaint_end=20,
        )
    ...
    with pytest.raises(ValidationError):
        EngineeringJobRequest(
            operation="repaint",
            asset_id="asset-1",
            transform_prompt="Regenerate this long section",
            repaint_start=0,
            repaint_end=121,
        )
...
def test_transform_request_exposes_no_raw_source_path_fields():
    body = EngineeringJobRequest(
        operation="cover",
        asset_id="project-owned-asset",
        transform_prompt="Create a soulful acoustic reinterpretation",
    )
...
def test_pro_cover_queues_project_asset_job(monkeypatch, tmp_path):
    ...
    body = EngineeringJobRequest(
        operation="cover",
        asset_id="asset-1",
        transform_prompt="Create a cinematic orchestral cover",
    )
...
def test_cover_worker_returns_only_project_relative_output(monkeypatch, tmp_path):
    ...
    class FakeAceStepClient:
        def cover(self, source_path, output_dir, *, prompt, strength):
            ...
            assert prompt == "Create a cinematic cover"
    ...
    result = run_engineering_job(
        ...
        EngineeringJobRequest(
            operation="cover",
            asset_id="asset-1",
            transform_prompt="Create a cinematic cover",
    ...
    with pytest.raises(ValueError, match="outside the member project"):
        run_engineering_job(
            ...
            EngineeringJobRequest(
                operation="cover",
                asset_id="asset-1",
                transform_prompt="Create a safe cover",
            ).model_dump(mode="json"),
        )
