...
def test_event_rejects_naive_timestamps_and_sensitive_audit_metadata():
    kwargs = dict(
        event_id="evt1", type="gift.sent", subject_type="gift", subject_id="gift1",
        occurred_at=NOW, correlation_id="corr1", source="internal",
    )
...
def test_idempotency_replay_conflict_and_outbox(tmp_path: Path):
    ...
    store.complete_idempotency(
        idempotency_key="idem1", request_hash=request_hash,
    ...
    event = EventEnvelope(
        event_id="evt1", type="ledger.posted", subject_type="ledger",
        subject_id="tx1", occurred_at=NOW, correlation_id="corr1", source="internal",
        idempotency_key="idem1",
    )
