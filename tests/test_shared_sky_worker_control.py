...
def test_scheduler_status_is_operational_but_redacts_worker_error_text(monkeypatch):
    class FakeWorker:
...
def test_manual_run_uses_canonical_worker_and_redacts_execution_error(monkeypatch):
    class FakeWorker:
        ...
        def heartbeat(self, **kwargs):  # pragma: no cover - only needed for exception path
...
def test_owner_scheduler_routes_are_mounted_on_esp_overlay():
    ...
    assert methods_by_path["/owner/shared-sky/api/scheduler/status"] == {"GET"}
    assert methods_by_path["/owner/shared-sky/api/scheduler/run-due"] == {"POST"}
