...
def test_non_stripe_order_fails_and_bound_order_recovers_same_verified_session(tmp_path, monkeypatch):
    store = MarketplaceOrderStore(tmp_path / "marketplace.sqlite3")
...
def test_bound_order_recovery_rejects_provider_identity_drift(tmp_path, monkeypatch):
    store = MarketplaceOrderStore(tmp_path / "marketplace.sqlite3")
    bound = _order(store)
    store.bind_provider_checkout(order_id=bound["id"], provider_checkout_reference="cs_bound")
    loaded = marketplace._load_order(store, bound["id"])
    monkeypatch.setattr(
        ...
        lambda config, path: {
            "id": "cs_bound",
            "mode": "payment",
            "client_reference_id": "different-buyer",
            "metadata": {
                "purchase_kind": "marketplace",
                "marketplace_order_id": bound["id"],
            },
