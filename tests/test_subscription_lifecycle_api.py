...
def _client(monkeypatch, store, ledger):
...
def test_subscription_status_requires_sign_in(tmp_path, monkeypatch):
    ...
    response = client.get("/membership/subscription")
...
def test_member_cancel_preserves_current_paid_access(tmp_path, monkeypatch):
    store, ledger, _user_id, token = _paid_user(tmp_path)
    client = _client(monkeypatch, store, ledger)
...
def test_verified_refund_requires_admin_and_never_returns_provider_references(tmp_path, monkeypatch):
    ...
    body = {
        "user_id": user_id,
        "payment_reference": "PAYMENT-REFERENCE-1234",
        "refund_reference": "REFUND-REFERENCE-5678",
    }
    ...
    response = client.post(
        "/admin/membership/record-verified-refund",
        json=body,
        headers={"x-lss-admin-key": "owner-secret"},
    )
    ...
    assert payload["provider_evidence_required"] is True
    assert "PAYMENT-REFERENCE-1234" not in str(payload)
