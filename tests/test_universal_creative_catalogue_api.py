...
def test_universal_routes_are_reachable_on_production_app_and_specific_before_catchall():
    ...
    assert "/command-center/api/universal-library/runtime-effects/owned" in paths
...
def test_purchase_api_debits_coins_and_owned_state_changes(coin_store):
    ...
    bought = universal_runtime_effect_purchase(
        "music.fx.limiter",
        EffectPurchaseRequest(idempotency_key="checkout-1"),
        _request(),
    )
    ...
    assert bought["individual_purchase_scope"] == "permanent_account_unlock"
...
def test_purchase_api_returns_payment_required_when_coin_balance_is_too_low(coin_store):
    with pytest.raises(HTTPException) as exc:
        universal_runtime_effect_purchase(
            "music.fx.limiter",
            EffectPurchaseRequest(idempotency_key="checkout-poor"),
            _request(),
        )
